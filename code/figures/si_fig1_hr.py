...
def plot(pooled, merged, log=print):
    ...
    ax = axes[0]
    healthy = pooled[pooled["hgroup"] == "HealthyControl"].dropna(subset=["HR_median", "CDC_median"])
    ax.scatter(healthy["CDC_median"], healthy["HR_median"], s=2, alpha=0.15, color=C_HC, rasterized=True)

    slope, intercept, r, p, se = stats.linregress(healthy["CDC_median"], healthy["HR_median"])
    ...
    ax.annotate(f"1/$e$ → HR = {predicted_hr:.0f} bpm", xy=(ONE_OVER_E, predicted_hr),
    ...
    ax.set_title("(a) 1/$e$ predicts healthy resting HR", fontsize=9, fontweight="bold", loc="left")
    ax.text(0.45, 50, f"R = {r:.3f}\np < {max(p, 1e-300):.0e}", fontsize=7, color="#555")
    ...
    for hr_bin in sorted(cn["HR_bin"].dropna().unique()):
        ...
        for tname, mask, color in [
            ...
            t = d[mask]
            if t["death"].sum() >= 5:
                bar_data.append(dict(hr_bin=str(hr_bin), tertile=tname,
                                     mort=100 * t["death"].mean(),
                                     se=100 * np.sqrt(t["death"].mean() * (1 - t["death"].mean()) / len(t)),
                                     color=color))
    ...
    if bar_data:
        ...
        ax.set_xticklabels([str(hb) for hb in hr_bins], fontsize=6, rotation=45)

    ax.set_xlabel("Heart rate stratum (bpm)", fontsize=9)
    ax.set_ylabel("All-cause mortality (%)", fontsize=9)
    ax.set_title("(b) CDC risk beyond HR screening", fontsize=9, fontweight="bold", loc="left")
